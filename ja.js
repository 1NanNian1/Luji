...
const weapons = {
    pistol: {
        ...
        icon: "fa-handgun",
        damage: 8, // 基础伤害
        fireRate: 500, // 射击间隔(毫秒)
        bulletSpeed: 12, // 子弹速度
        bulletColor: "#3B82F6", // 子弹颜色
        unlockCost: 0, // 解锁价格
        upgradeCost: [50, 100, 150, 200], // 升级价格
        upgradeDamage: [8, 12, 18, 25], // 升级后伤害
        upgradeFireRate: [500, 420, 350, 280], // 升级后射击间隔
    ...
    rifle: {
        ...
        upgradeDamage: [25, 32, 40, 50],
        upgradeFireRate: [250, 210, 180, 150],
        isUnlocked: false
    },
    flamethrower: {
        name: "喷火器",
...
function drawCharacter() {
    ...
    ctx.beginPath();
    ctx.arc(screenX, screenY, character.width / 2, 0, Math.PI * 2);
    ctx.fill();
    
    // 角色眼睛
    ctx.fillStyle = 'white';
    ctx.beginPath();
    ctx.arc(screenX - character.width * 0.2, screenY - character.width * 0.3, character.width * 0.1, 0, Math.PI * 2);
    ctx.arc(screenX + character.width * 0.2, screenY - character.width * 0.3, character.width * 0.1, 0, Math.PI * 2);
    ctx.fill();
    
    ctx.fillStyle = 'black';
    ctx.beginPath();
    ctx.arc(screenX - character.width * 0.15, screenY - character.width * 0.35, character.width * 0.05, 0, Math.PI * 2);
    ctx.arc(screenX + character.width * 0.25, screenY - character.width * 0.35, character.width * 0.05, 0, Math.PI * 2);
    ctx.fill();
...
function drawEnemies() {
    enemies.forEach(enemy => {
        ...
        if (enemy.type === 'ranged') {
            // 远程敌人用方形表示
            ctx.rect(screenX - enemy.width / 2, screenY - enemy.height / 2, enemy.width, enemy.height);
        } else {
        ...
        ctx.fillStyle = 'black';
        ctx.beginPath();
        ctx.arc(screenX - enemy.width * 0.2, screenY - enemy.width * 0.3, enemy.width * 0.1, 0, Math.PI * 2);
        ctx.arc(screenX + enemy.width * 0.2, screenY - enemy.width * 0.3, enemy.width * 0.1, 0, Math.PI * 2);
        ctx.fill();
...
function updateSkillCooldowns(timestamp) {
    // This can be used to update UI for cooldowns
}
